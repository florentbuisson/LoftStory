public class Erratique extends Neuneu {
	...
	void bougeDeLa() {
		...
		this.setMaCase(maison.getCase(getLaCase().getWPosition() + deplacement
				/ 3, getLaCase().getHPosition() + deplacement % 3));
	}
	...
	void fornique() {
		boolean premierTrouve = true;
		// S'accoupler doit �tre plus int�rressant que la n�crophilie: on cr�e
		// donc un erratique mort.
		// Ceci est du au manque de souplesse de java quant � la
		// non-initialisation.
	...
	Neuneu accouplement(Neuneu partenaire) {
		Erratique bebeNeuneu = new Erratique(maison,
				getLaCase().getWPosition(), getLaCase().getHPosition(),
				new String("Erratique"
